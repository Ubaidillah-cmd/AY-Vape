    ...
    <div class="footer-grid">
      ...
      <div>
        ...
        <div class="footer-links">
          <a href="../public/index.php"   class="footer-link">Home</a>
          <a href="../public/about.php"   class="footer-link">Tentang Kami</a>
          <a href="../public/contact.php" class="footer-link">Kontak</a>
          <a href="../customer/cart.php"  class="footer-link">Keranjang</a>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="footer-links">
          <a href="../public/cara_order.php" class="footer-link">Cara Order</a>
          <a href="../public/pengiriman.php"  class="footer-link">Pengiriman</a>
          <a href="../public/pembayaran.php"  class="footer-link">Pembayaran</a>
          <a href="../public/faq.php"         class="footer-link">FAQ</a>
        </div>
      </div>
...
function subscribeNewsletter() {
  ...
  if (!email) {
    shakeInput(input);
    showFooterToast('⚠️ Masukkan email kamu dulu!', 'warn');
    input.focus();
    return;
  }
  ...
  if (!emailRegex.test(email)) {
    shakeInput(input);
    showFooterToast('⚠️ Format email tidak valid!', 'warn');
    input.focus();
    return;
  }
  ...
  .then(d => {
    if (d.ok) {
      ...
      showFooterToast('🎉 Subscribe berhasil! Admin sudah diberitahu.', 'success');
    } else if (d.type === 'duplicate') {
      btn.innerHTML  = 'Subscribe →';
      btn.disabled   = false;
      input.disabled = false;
      shakeInput(input);
      showFooterToast('ℹ️ Email ini sudah terdaftar!', 'info');
    } else {
      btn.innerHTML  = 'Subscribe →';
      btn.disabled   = false;
      input.disabled = false;
...
function shakeInput(el) {
  ...
  el.offsetHeight;
  el.style.animation   = 'footerShake 0.4s ease';
...
function showFooterToast(text, type) {
  ...
  Object.assign(toast.style, {
    ...
    animation:    'footerToastIn .3s cubic-bezier(0.34,1.56,0.64,1)',
    whiteSpace:   'nowrap',
